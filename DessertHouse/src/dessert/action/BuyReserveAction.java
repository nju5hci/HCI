...
public class BuyReserveAction extends BaseAction{
	...
	public String execute() throws Exception {
		...
		List<Reserve> reserves=memberService.getBookReservesByMid(mid);
		for(int i=0;i<reserves.size();i++){
			...
			dessert.setDessertStock(dessertStock-reserves.get(i).getDessertNum());
			dessertService.updateDessert(dessert);
			
			double price=dessert.getDessertPrice();
			MemberCard memberCard=memberCardService.getCardById(mid);
			int level=memberCard.getLevel();
			//根据不同的等级来打折
			if(level==1){
				price=price*Configure.ONE_DISCOUNT;
			}else if(level==2){
				price=price*Configure.TWO_DISCOUNT;
			}else if(level==3){
				price=price*Configure.THREE_DISCOUNT;
			}
